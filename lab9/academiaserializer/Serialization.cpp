...
namespace academia {
    ...
    void BuildingRepository::StoreAll(Serializer *serializer) const {
        ...
        vector<reference_wrapper<const Serializable>> vec;
        for(int i=0; i<buildings_.size();i++)
            vec.emplace_back(buildings_[i]);
        serializer->ArrayField("buildings",vec);
    ...
    experimental::optional<Building> BuildingRepository::operator[](int index) const {
        ...
        for(int i=0; i<buildings_.size();i++)
        {
            if(buildings_[i].Id()==index)
                return std::experimental::make_optional(buildings_[i]);
        }
